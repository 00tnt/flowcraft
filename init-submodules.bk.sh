...
# GITHUB_ACCESS_TOKEN is necessary
...
process_submodules() {
  ...
  while IFS= read -r INTERNAL_NAME; do
    ...
    git submodule add -b $SUBMODULE_BRANCH "https://$GITHUB_ACCESS_TOKEN@github.com/$SUBMODULE_OWNER/$SUBMODULE_REPO.git" $SUBMODULE_PATH

    echo "Cleaning up git configuration...$INTERNAL_NAME"
    git config --local --remove-section submodule."$INTERNAL_NAME"
    ...
    # This is specific to aioz-ui, you can skip this if you are not using aioz-ui
    cd $SUBMODULE_PATH
